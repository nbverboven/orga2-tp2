...
; RCX := uint8_t* dst, R8 := uint32_t dstw, R9 := uint32_t dsth
	...
	.ciclo_borde_derecho:
		...
		movq         xmm0, [rbx]         ; xmm0 <- [ ----- | ----- | src[n-1][m-1] | src[n-1][m-2] ]
		movq         xmm1, [rbx + 4*rsi] ; xmm1 <- [ ----- | ----- | src[n-2][m-1] | src[n-2][m-2] ]

		punpcklbw    xmm0, xmm8          ; xmm0 <- [ src[n-1][m-1] | src[n-1][m-2] ]
		punpcklbw    xmm1, xmm8          ; xmm1 <- [ src[n-2][m-1] | src[n-2][m-2] ]

		movdqu       xmm2, xmm0          ; xmm2 = xmm0
		movdqu       xmm3, xmm1          ; xmm3 = xmm1

		paddw        xmm2, xmm1          ; xmm2 <- [ src[n-1][m-1] + src[n-2][m-1] | src[n-1][m-2] + src[n-2][m-2] ]
		movdqu       xmm4, xmm2          ; xmm4 = xmm2
		psrldq       xmm4, 8             ; xmm4 <- [    0    | src[n-1][m-1] + src[n-2][m-1] ]
		paddw        xmm4, xmm2          ; xmm4 <- [ ------- | src[n-1][m-1] + src[n-2][m-1] + src[n-1][m-2] + src[n-2][m-2] ]

		psrlw        xmm2, 1             ; xmm2 <- [ (src[n-1][m-1] + src[n-2][m-1]) / 2 | (src[n-1][m-2] + src[n-2][m-2]) / 2 ]
		psrlw        xmm4, 2             ; xmm4 <- [ ------- | (src[n-1][m-1] + src[n-2][m-1] + src[n-1][m-2] + src[n-2][m-2]) / 4 ]

		movdqu       xmm5, xmm1          ; xmm1 <- [ src[n-2][m-1] | src[n-2][m-2] ]
		psrldq       xmm5, 8             ; xmm5 <- [       0       | src[n-2][m-1] ]
		paddw        xmm5, xmm1
		psrlw        xmm5, 1             ; xmm5 <- [ ------------- | (src[n-2][m-1] + src[n-2][m-2]) / 2 ]

		packuswb     xmm1, xmm8          ; xmm1 <- [       0       |        0       |            src[n-2][m-1]            |             src[n-2][m-2]           ] 
		packuswb     xmm5, xmm8          ; xmm5 <- [       0       |        0       |            -------------            | (src[n-2][m-1] + src[n-2][m-2]) / 2 ]
		packuswb     xmm2, xmm8          ; xmm2 <- [       0       |        0       | (src[n-1][m-1] + src[n-2][m-1]) / 2 | (src[n-1][m-2] + src[n-2][m-2]) / 2 ]
		packuswb     xmm4, xmm8          ; xmm4 <- [       0       |        0       |            -------------            | (src[n-1][m-1] + src[n-2][m-1] + src[n-1][m-2] + src[n-2][m-2]) / 4 ]

		; [ 3 | 2 | 1 | 0 ] => [ 1 | 1 | 2 | 0 ] (01 01 10 00 == 0101 1000 == 0x58)

		pslldq       xmm4, 8
		por          xmm2, xmm4          ; xmm2 <- [ ------- | (src[n-1][m-1] + src[n-2][m-1] + src[n-1][m-2] + src[n-2][m-2]) / 4 | (src[n-1][m-1] + src[n-2][m-1]) / 2 | (src[n-1][m-2] + src[n-2][m-2]) / 2 ]
		pshufd       xmm2, xmm2, 0x58    ; xmm2 <- [ (src[n-1][m-1] + src[n-2][m-1]) / 2 | (src[n-1][m-1] + src[n-2][m-1]) / 2 | (src[n-1][m-1] + src[n-2][m-1] + src[n-1][m-2] + src[n-2][m-2]) / 4 | (src[n-1][m-2] + src[n-2][m-2]) / 2 ]

		pslldq       xmm5, 8
		por          xmm5, xmm1          ; xmm5 <- [ ------------- | (src[n-2][m-1] + src[n-2][m-2]) / 2 | src[n-2][m-1] | src[n-2][m-2] ]
		pshufd       xmm5, xmm5, 0x58    ; xmm5 <- [ src[n-2][m-1] | src[n-2][m-1] | (src[n-2][m-1] + src[n-2][m-2]) / 2 | src[n-2][m-2] ]
	...
	.fin:
		...
		; Esta iteración corresponde a la esquina inferior derecha.
		; La hago a mano porque es una mezcla entre la forma de operar con el borde inferior
		; y el derecho.
		lea          rbx, [rdi + 4*rsi - 8] ; rbx <- &src[srch-1][srcw-2]
